...
def test_benchmark_high_quality_grounding():
    ...
    response = orchestrate_therapy_generation("PGX-001", "opioid pain response research")
...
def test_benchmark_low_quality_grounding():
    ...
    assert any("insufficient research evidence" in step.summary for step in response.agent_steps if step.agent == "TargetSelection")
...
def test_benchmark_validation_revision_loop():
    ...
    response = orchestrate_therapy_generation("PGX-001", "opioid research simulation", max_iterations=3)
