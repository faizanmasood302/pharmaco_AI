...
def test_risk_matrix_poor_metabolizer_codeine(mock_poor_metabolizer_patient):
    ...
    res = assess_prescription("TEST-PM", "Codeine", patient=mock_poor_metabolizer_patient)
...
def test_risk_matrix_multi_enzyme_hydrocodone(mock_poor_metabolizer_patient):
    ...
    mock_poor_metabolizer_patient["cyp_profiles"].append({
        "gene": "CYP3A4", "diplotype": "*22/*22", "phenotype": "Poor Metabolizer", "activity_score": "0.0"
    })
    res = assess_prescription("TEST-PM", "Hydrocodone", patient=mock_poor_metabolizer_patient)
...
def test_risk_matrix_clopidogrel_critical(mock_poor_metabolizer_patient):
    ...
    mock_poor_metabolizer_patient["cyp_profiles"] = [
        {"gene": "CYP2C19", "diplotype": "*2/*2", "phenotype": "Poor Metabolizer", "activity_score": "0.0"}
    ]
    res = assess_prescription("TEST-PM", "Clopidogrel", patient=mock_poor_metabolizer_patient)
    assert res.risk_level == RiskLevel.CRITICAL
    assert res.flagged is True
