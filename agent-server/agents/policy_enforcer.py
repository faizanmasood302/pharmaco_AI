...
def enforce_policy(
    medication: str, risk_level: str, rationale: str
) -> tuple[str, str, int]:
    """
    ...
    policy_path = os.path.join(
        base_dir, "clinical_logic", "Override_and_Audit_Policy.md"
    )
        ...
        try:
            ...
            completion = _groq.chat.completions.create(
                messages=[
                    {
                        "role": "system",
                        "content": "You are a medical compliance officer. Be strict and refer only to the provided policy.",
                    },
            ...
            if "APPROVED" in raw_output.upper():
                verdict = "APPROVED"
            elif "BLOCKED" in raw_output.upper():
                verdict = "BLOCKED"
            else:
                verdict = "CONDITIONAL"
            analysis = raw_output
